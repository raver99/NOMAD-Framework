...
[Claude Code Skills](https://github.com/raver99/NOMAD-Framework/blob/main/KnowledgeBase/ClaudeCode/claude-code-skills.md).
...
### 10.1 A Skill That Needs Setup Ships the Setup

Some skills only work once something exists in the target project: a statement in `CLAUDE.md`, a
config file, a directory layout, an installed CLI, a credential. Left as instructions in a README,
that setup gets paraphrased, half-finished or skipped — and the failure is **silent**. The skill
looks installed and misbehaves later, by which point nobody connects the two.

Ship it as a **one-time, explicitly invoked skill** in the same plugin. Explicit invocation is the
point (§5.4): the person has to be able to name it, and it must never fire on its own.

What makes one correct:

- **Guard every step.** Write it assuming it is being run a second time. Detect what is already
  done, refuse to overwrite anything holding content, refuse to append a section twice. Where the
  existing state and the intended state disagree, **stop and report rather than merge** — which of
  the two is right is the user's decision, not the skill's.
- **Copy shipped assets verbatim.** Text regenerated from memory is a paraphrase, and one convention
  stated three slightly different ways across three projects is worse than one stated badly. Keep
  the canonical text in `assets/` and copy it.
- **Verify instead of assuming.** A step that installs a tool checks that it runs; a step that
  configures credentials checks that they authenticate. Setup that reports success without checking
  is worse than no setup, because it converts a loud failure into a quiet one.
- **Never write a secret into the repository.** Prompt for it, put it where that ecosystem keeps
  secrets, and confirm the file it lands in is ignored by version control.
- **Report what remains manual.** A credential only a human can obtain, an approval, a paid account
  — say so at the end, or the user believes setup finished when it did not.
- **Scope `allowed-tools` tightly** (§4). A setup skill writes files, edits project configuration and
  may run installers, which makes it the most side-effecting thing in a package.

❌ **Do not write one when there is nothing to set up.** A skill whose only output is "you are ready"
is overhead, and it teaches people to ignore the next one.

> **Prefer removing the need to automating it.** Setup that exists because an artifact cannot
> describe itself is a design problem being papered over. A file that states its own conventions in
> its own header needs no instruction installed beside it, and it serves the people who install
> nothing.
...
- Does it require something to exist in the target project that nothing installs? (§10.1)
